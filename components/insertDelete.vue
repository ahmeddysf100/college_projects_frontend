...
const selectSubject = ['ashkal', 'lost numbers', 'verbal', 'Sequences', 'Arithmetic and logical operations']
...
const addOption = () => {
  if (tempOptions.value && !options.value.includes(tempOptions.value)) {
    options.value.push({ A_Text: tempOptions.value, isCorrect: selectBox.value.toString() }); // Access the value property
...
const optionsJsonString = computed(() => JSON.stringify(options.value, null, 2));
...
async function onSubmitinsert() {
  if (insertForm.answers === null && insertForm.correctAnswer === null) {
    useModel.setModalValues(true, 'Bad Request', 'You cannot send a blank form')
  } else {
    ...
    if (useDashboard.insertRes) {
      useModel.setModalValues(true, 'Question Added Successfully', useDashboard.insertRes)
...
async function onDeletion() {
  ...
  if (useDashboard.questionById.length > 4) {
    ...
    useModel.setModalValues(true, 'Confirm deletion', useDashboard.questionById, true)
  }
  else {
    useModel.setModalValues(true, 'This question not exist', useDashboard.questionById.response, false)
  }
      ...
      <UCard>
        ...
        <UForm @submit="onSubmitinsert">
          ...
          <UFormGroup label="Email" name="email" class="mb-3" required>
            <UInput v-model="insertForm.email" />
          </UFormGroup>
          ...
          <UFormGroup v-if="insertForm.type === 'multiple options'" class="mb-4">
            <UButton size="xs" label="Add Options" class="mb-3" @click="isOpen = true" />
            <UTextarea readonly color="indigo" variant="outline" :value="optionsJsonString" @input="updateOptions"
              ...
              <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
                ...
                <UFormGroup class="mb-10 " label="Enter options" required>
                  <div class="grid grid-rows-1 grid-cols-2 gap-1 ">
                    <UInput class="mb-2 justify-stretch" v-model="tempOptions" />
                    <UCheckbox class="" v-model="selectBox" name="notifications" label="is true" />
                    <p class=" text-left text-red-500 rounded-sm mb-2">{{ optionMessage }}</p>
                  </div>
                  <UButton class=" justify-self-start" label="Add" @click="addOption" />
                </UFormGroup>

                <div class="flex flex-wrap gap-1">
                  <div v-for="option in options" :key="option">
                    <p class=" break-all rounded bg-slate-600 mt-1 ml-1 px-1 cursor-pointer text-xl hover:bg-rose-700"
                      @click="removeOption(option)">{{ option }}</p>
                  </div>
                </div>
              </UCard>
            </UModal>
          ...
          <UFormGroup label="Question Text" name="Q_text" class="mb-3">
            <UTextarea autoresize v-model="insertForm.Q_text" />
          </UFormGroup>

          <UFormGroup label="Question image" name="image1" class="mb-3">
            <UInput type="file" @change="handleFileUpload1" />
          ...
          <UFormGroup label="Explanation Text" name="explanationText" class="mb-3">
            <UTextarea autoresize v-model="insertForm.explanationText" />
          </UFormGroup>

          <UFormGroup label="Answer image" name="image2" class="mb-3">
            <UInput type="file" @change="handleFileUpload2" />
    ...
    <template #delete="{ item }">
      <UCard @submit.prevent="onDeletion">
        <template #header>
          ...
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Delete question. After saving, you will get the question information to confirm the deletion. </p>
        </template>
