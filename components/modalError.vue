...
const deletion = async () => {
  ...
  if (useDashboard.D_question) {
    ...
    useModel.setModalValues(true, 'Done !', `The question has been deleted. The page will be updated within`, false);
  ...
  <div>
    ...
    <UModal v-model="isOpen" prevent-close>
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        ...
        <p v-else class=" text-base font-semibold text-red-400">{{ message }} <span v-if="countNum" class=" animate-spin text-white">{{ countdown }} </span> {{ countdown > 1 ? 'seconds' : 'second' }}</p>
