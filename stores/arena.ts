...
import type { Arena, arenaForDto } from "~/createArena";
...
export const useMyCreateArenaStore = defineStore("arena", () => {
  ...
  const set_admin_arena_token = (data?: string) => (admin_arena_token.value = data);
  const set_admin_arena_id = (data?: string) => (admin_arena_id.value = data);


  const createArena = async (data: arenaForDto ) => {
    try {
      const req = await $fetch<CreateArena>("http://192.168.31.170:3333/arena", {
        method: "POST",
        body: data,
        headers: {
