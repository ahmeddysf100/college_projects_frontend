...
import type { AnswerExplanationItem, FilteredItem, OriginalItem, answersItem } from '~/types';
...
const dataTable = async () => {
  ...
  filteredData.value = rawData.map((item: any) => {
    const { id, Q_imageUrl, Q_text, correctAnswer, answers, AnswerExplanation } = item;
    ...
    const answersFilter: answersItem[] = answers.map(
      (val: answersItem) => ({
        A_text: val.A_text,
    ...
    return {
      ...
      explanationText: explanation.length > 0 ? explanation[0].explanationText : null,
...
const fetchImage = async (imageUrl: any) => {
  try {
    ...
    blob.value = URL.createObjectURL(useDash.image);
  } catch (error) {
    blob.value = ''
      ...
      <UTable :columns="columns" :rows="rows">
        ...
        <template #answers-data="{ row }">
          <UPopover v-if="row.answers.length > 0" mode="hover"
            ...
            <template #panel>
              <div class="p-2">
                <UTextarea autoresize :model-value="JSON.stringify(row.answers, null, 2)" />
              </div>
            </template>
          </UPopover>
          <p v-else>null</p>
              ...
              <div class="p-2 ">
                ...
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="300" height="150" viewBox="0 0 300 150">
                  ...
                  <text x="50%" y="50%" font-family="Arial, sans-serif" font-size="24" fill="#ecf0f1" text-anchor="middle"
                    alignment-baseline="middle">Not Found</text>
                </svg>
              </div>
            </template>
          </UPopover>
